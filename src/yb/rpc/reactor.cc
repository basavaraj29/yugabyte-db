...
void Reactor::ScanForStuckOutboundCalls(CoarseTimePoint now) {
  ...
  // Collect all connections for which there are stuck outbound calls, and report details for those
  // connections at the end of this function.
  std::set<ConnectionPtr> reported_connections;
  ...
  while (!index.empty() && index.begin()->next_check_time <= now) {
    ...
    if (conn) {
      reported_connections.insert(conn);
    }
    ...
    if (forced_expiration) {
      if (conn) {
        conn->ExpireCallById(call->call_id());
      } else {
  ...
  for (const auto& conn : reported_connections) {
    LOG_WITH_PREFIX(WARNING)
        << "Connection with stuck OutboundCalls: " << conn->DebugStringOnReactorThread();
  }
}
