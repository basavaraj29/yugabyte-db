...
void OutboundCall::SetFinished() {
  ...
  bool invoke_callback = SetState(RpcCallState::FINISHED_SUCCESS);
  if (invoke_callback) {
    InvokeCallback();
  }
}
...
string OutboundCall::ToString() const {
  ...
  out << "RPC call " << pointer_cast<const void*>(this);
...
std::string OutboundCall::DebugString() const {
  ...
  out << ", connection: ";
  out << (conn ? conn->ToString() : (conn_raw_ptr ? Format("$0", conn_raw_ptr) : "N/A"));
