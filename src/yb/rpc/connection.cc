...
Result<size_t> Connection::DoQueueOutboundData(OutboundDataPtr outbound_data, bool batch) {
  ...
  auto result = PREDICT_FALSE(
    ...
    : stream_->Send(outbound_data);

  if (!result.ok()) {
    outbound_data->Transferred(result.status(), shared_from_this());
...
std::string Connection::DebugStringOnReactorThread() const {
  ...
  out << "; active calls (" << active_calls_.size() << "):\n";
  int i = 1;
  auto now = CoarseMonoClock::Now();
  for (auto& active_call : active_calls_) {
    out << "    " << i << ". " << active_call.ToString(now) << "\n";
    i++;
  }
  auto result = out.str();
...
void Connection::ExpireCallById(int32_t call_id) {
